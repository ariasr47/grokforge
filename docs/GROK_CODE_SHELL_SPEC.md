...
**Related:** `unified-agent-shell-design.html`
...
Grok-first delivers daily value immediately and forces the shell (permissions, diffs, ACP host) to be real before multi-agent wiring.
...
```
Shell UI (Tauri/Electron + React)
    → ACP client (stdio JSON-RPC)
        → grok-acp (child process, cwd=workspace)
            → xAI (OAuth sub-pool | API key)
            → local tools (read/grep/patch/shell)
```
...
| Topic | Proposal |
|-------|----------|
| Framework | **Tauri 2** unless Electron is required for speed of delivery |
