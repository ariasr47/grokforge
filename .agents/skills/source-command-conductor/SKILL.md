---
name: "source-command-conductor"
description: "Re-boot the delivery ascent conductor (orchestrator), reconstructing state from disk."
---

# source-command-conductor

Use this skill when the user asks to run the migrated source command `conductor`.
...
`spire run tech`) for a cold open of `spire:tech`; use this `/conductor` command to re-boot the
same mediator mid-ascent after a checkpoint. **Every message you send me is an operator card per
`.Codex/OPERATOR_REPORTS.md` (ORCHESTRATOR §5/§7): chip headline, ≤10 plain-language lines above
...
- .Codex/ORCHESTRATOR.md       (your driver — operating loop §0, gateway catalog §3, invariants §6)
- .Codex/OPERATOR_REPORTS.md    (your user-facing voice — every report is one of its cards)
- the INVARIANT FLOOR of the project context file — run
  `node .Codex/tools/gates.mjs context_for --print` (no feature): with no tags it emits only the
  ...
  (GATE I; the GATE S tally is `node .Codex/tools/gates.mjs ledger_tally`). Boot must stay ~22K:
...
`node .Codex/tools/continuity.mjs mark-consumed .spire/clusters/tech/state/<branch-slug>/RESUME.md`
...
`node .Codex/tools/continuity.mjs list`. If the scoped RESUME is absent — including when this
...
never a long-lived session. The design stage is the **council** (`/council {FEATURE}`, GATE C): three
...
**PROPOSE** that I continue via cluster entry or a fresh `/conductor` re-boot (which will read that
