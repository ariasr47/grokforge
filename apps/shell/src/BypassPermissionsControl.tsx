...
export function BypassPermissionsControl({ sessionId, unlocked, available, active = false, blockedReason, onActiveChange }: BypassPermissionsControlProps) {
  ...
  if (activeState) return <div className="bypass-danger" role="status"><strong>Bypass permissions active</strong><p>Approvals are bypassed for this session. OS permissions and destructive-operation circuit breakers still apply.</p><button type="button" onClick={exit} disabled={busy}>Exit Bypass permissions</button>{error && <p role="alert">{error}</p>}</div>;
  ...
  if (!unlocked) return <div className="bypass-control"><strong>Advanced permissions</strong>{confirming ? <div role="alertdialog" aria-label="Unlock Bypass permissions?"><h3>Unlock Bypass permissions?</h3><p>Bypass permissions can change or delete files, run commands, use the network, and access paths outside the selected workspace without asking. Audit history remains visible, but command side effects may not be reversible.</p><label><input type="checkbox" checked={acknowledged} onChange={e => setAcknowledged(e.target.checked)} /> I understand this affects every local Forge session I explicitly place in Bypass permissions.</label><button type="button" onClick={() => void unlock()} disabled={!acknowledged || busy}>Unlock on this machine</button><button type="button" onClick={() => setConfirming(false)}>Cancel</button></div> : <button type="button" onClick={() => setConfirming(true)} disabled={busy}>Unlock Bypass permissions</button>}{error && <p role="alert">{error}</p>}</div>;
  return <div className="bypass-control"><strong>Advanced permissions</strong>{confirming ? <div role="alertdialog" aria-label="Confirm Bypass permissions"><h3>Enable Bypass permissions?</h3><p>Bypass permissions — Runs tools without approval, including commands, network use, and paths outside this workspace. OS permissions and destructive-operation circuit breakers still apply.</p><label><input type="checkbox" checked={acknowledged} onChange={e => setAcknowledged(e.target.checked)} /> I understand this affects every local Forge session I explicitly place in Bypass permissions.</label><button type="button" onClick={() => void activate()} disabled={!acknowledged || busy}>Enable Bypass permissions</button><button type="button" onClick={() => setConfirming(false)}>Cancel</button></div> : <button type="button" onClick={() => setConfirming(true)} disabled={busy}>Enable Bypass permissions</button>}{error && <p role="alert">{error}</p>}</div>;
}
