...
async function waitMembers(session: AgentSession, serverId: string, ms = 8000) {
  const deadline = Date.now() + ms;
  while (Date.now() < deadline) {
    const members = session.getState().mcpServers.members;
    if (members?.some((m) => m.serverId === serverId)) return members;
    await new Promise((r) => setTimeout(r, 25));
  }
  throw new Error(`timeout waiting for ${serverId}: ${JSON.stringify(session.getState().mcpServers)}`);
}

describe("vendor-mcp-visible isolation", () => {
  it("two concurrent sessions do not share MCP rosters", async () => {
    const home = await fs.mkdtemp(path.join(os.tmpdir(), "mcp-iso-"));
    ...
    process.env.GROKFORGE_VENDOR_FIXTURE = "mcp-server-multi";
    ...
    try {
      ...
      await waitMembers(a, "railway");
      await waitMembers(a, "chrome-devtools");
      await waitMembers(b, "railway");
      await waitMembers(b, "chrome-devtools");
      ...
      const idsA = (a.getState().mcpServers.members ?? []).map((m) => m.serverId).sort();
      const idsB = (b.getState().mcpServers.members ?? []).map((m) => m.serverId).sort();
      assert.deepEqual(idsA, ["chrome-devtools", "railway"]);
      assert.deepEqual(idsB, ["chrome-devtools", "railway"]);
  ...
  it("foreign connectionGeneration MCP frame drops before journal apply", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "mcp-gen-"));
    try {
      ...
      assert.equal(events.filter((e) => e.type === "mcp_server_update").length, 1);
    } finally {
