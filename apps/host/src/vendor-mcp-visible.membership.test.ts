...
async function setup(opts: {
  ...
  const home = await fs.mkdtemp(path.join(os.tmpdir(), "mcp-mem-"));
...
async function waitMcp(
  session: AgentSession,
  pred: (fact: ReturnType<AgentSession["getState"]>["mcpServers"]) => boolean,
  label: string,
  ms = 8000,
) {
  const deadline = Date.now() + ms;
  let last = session.getState().mcpServers;
  while (Date.now() < deadline) {
    last = session.getState().mcpServers;
...
describe("vendor-mcp-visible membership dispositions", () => {
  it("Chat session → absent_for_non_code_or_non_vendor, members null, codeAgent null", async () => {
    const home = await fs.mkdtemp(path.join(os.tmpdir(), "mcp-chat-"));
    process.env.GROKFORGE_CHANNEL = "test";
    ...
    try {
      ...
      assert.equal(state.mcpServers.disposition, "absent_for_non_code_or_non_vendor");
      assert.equal(state.mcpServers.members, null);
    } finally {
  ...
  it("Code fallback / hard_fail → absent; prior vendor members not projected live", async () => {
    const vendor = await setup({ fixture: "mcp-server" });
    try {
  ...
  it("Code+vendor catch-up in flight → hydrating; prior ready members retained", async () => {
    ...
    try {
      ...
      await waitMcp(
        ctx.session,
        (c) => c.disposition === "ready" && c.members?.some((m) => m.serverId === "railway") === true,
        "first ready",
      );
      ...
      ctx.session.on((ev) => {
        if (ev.type === "state" && ev.state.mcpServers.disposition === "hydrating") {
          hydrating.push({
            disposition: ev.state.mcpServers.disposition,
            members: ev.state.mcpServers.members,
          });
        }
      });
      await ctx.session.prompt("two", "auto", { clientSessionId: "member01" });
      assert.ok(
        hydrating.some((h) => Array.isArray(h.members) && (h.members as Array<{ serverId: string }>).some((m) => m.serverId === "railway")),
  ...
  it("ready + zero MCP → ready + []", async () => {
    ...
    try {
      ...
      assert.notEqual(ctx.session.getState().mcpServers.disposition, "absent_for_non_code_or_non_vendor");
      await ctx.session.prompt("hi", "auto", { clientSessionId: "member01" });
      const cat = await waitMcp(
        ctx.session,
  ...
  it("journal obtain failure → obtain_failed keeps last-ready members", async () => {
    ...
    try {
      ...
      const priorId = ctx.session.getState().mcpServers.members?.[0]?.serverId;
      ...
      assert.equal(ctx.session.getState().mcpServers.disposition, "obtain_failed");
      assert.equal(ctx.session.getState().mcpServers.members?.length, 1);
      assert.equal(ctx.session.getState().mcpServers.members?.[0]?.serverId, priorId);
    } finally {
      await ctx.session.shutdown().catch(() => undefined);
      await fs.rm(ctx.home, { recursive: true, force: true }).catch(() => undefined);
    }
  ...
  it("malformed individual MCP frame does not flip obtain_failed", async () => {
  ...
  it("reload/replay restores journaled identities + vouched Idle/Error", async () => {
    const ctx = await setup({ fixture: "mcp-server-first-error", sessionId: "reload01" });
    try {
      await ctx.session.openWorkspace(ctx.workspace);
      const run = await ctx.session.prompt("go", "auto", { clientSessionId: "reload01" });
      await waitMcp(
        ctx.session,
        (c) => c.members?.some((m) => m.serverId === "figma" && m.status === "error") === true,
        "journaled error",
      );
      await ctx.session.shutdown().catch(() => undefined);
      ...
      try {
        ...
        const restored = await waitMcp(
          session2,
          (c) => c.disposition === "ready" && c.members?.some((m) => m.serverId === "figma" && m.status === "error") === true,
          "restored members",
        );
        assert.equal(restored.members?.length, 1);
        assert.equal(restored.members?.[0]?.name, "figma");
        const replay = await session2.replayRun(run.runId, "reload01");
        assert.ok(replay.events.some((e) => e.type === "mcp_server_update" && e.payload.kind === "mcp_server_update"));
      } finally {
  ...
  it("disk / connectors catalog alone does not invent MCP members", async () => {
    const ctx = await setup({ fixture: "ok" });
    try {
      await fs.mkdir(path.join(ctx.home, ".grok"), { recursive: true });
      await fs.writeFile(
        path.join(ctx.home, ".grok", "config.toml"),
        "[mcp_servers.ghost]\ncommand = \"echo\"\n",
        "utf8",
      );
      await fs.writeFile(
        path.join(ctx.dataDir, "connectors.json"),
        JSON.stringify({ tokens: { gmail: "x" } }),
        "utf8",
      );
      await ctx.session.openWorkspace(ctx.workspace);
      await ctx.session.prompt("hi", "auto", { clientSessionId: "member01" });
