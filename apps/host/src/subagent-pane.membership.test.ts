...
async function setup(opts: {
  ...
  const home = await fs.mkdtemp(path.join(os.tmpdir(), "subagent-mem-"));
...
async function waitChildAgents(
  session: AgentSession,
  pred: (fact: ReturnType<AgentSession["getState"]>["childAgents"]) => boolean,
  label: string,
  ms = 8000,
) {
  const deadline = Date.now() + ms;
  let last = session.getState().childAgents;
  while (Date.now() < deadline) {
    last = session.getState().childAgents;
...
describe("subagent-pane membership dispositions", () => {
  it("Chat session → absent_non_code_or_non_vendor, members null, codeAgent null", async () => {
    const home = await fs.mkdtemp(path.join(os.tmpdir(), "subagent-chat-"));
    process.env.GROKFORGE_CHANNEL = "test";
    ...
    try {
      ...
      assert.equal(state.childAgents.disposition, "absent_non_code_or_non_vendor");
      assert.equal(state.childAgents.members, null);
    } finally {
  ...
  it("Code fallback / hard_fail → absent; prior vendor members not projected live", async () => {
    const vendor = await setup({ fixture: "child-agent" });
    try {
  ...
  it("hard_fail → absent_non_code_or_non_vendor", async () => {
  ...
  it("Code+vendor catch-up in flight → hydrating; prior ready members retained", async () => {
    ...
    try {
      ...
      await waitChildAgents(
        ctx.session,
        (c) => c.disposition === "ready" && c.members?.some((m) => m.childId === "child-1") === true,
        "first ready",
      );
      ...
      ctx.session.on((ev) => {
        if (ev.type === "state" && ev.state.childAgents.disposition === "hydrating") {
          hydrating.push({
            disposition: ev.state.childAgents.disposition,
            members: ev.state.childAgents.members,
          });
        }
      });
      await ctx.session.prompt("two", "auto", { clientSessionId: "member01" });
      assert.ok(
        hydrating.some((h) => Array.isArray(h.members) && (h.members as Array<{ childId: string }>).some((m) => m.childId === "child-1")),
  ...
  it("ready + zero children → ready + []", async () => {
    ...
    try {
      ...
      assert.notEqual(ctx.session.getState().childAgents.disposition, "absent_non_code_or_non_vendor");
      await ctx.session.prompt("hi", "auto", { clientSessionId: "member01" });
      const cat = await waitChildAgents(
        ctx.session,
  ...
  it("journal obtain failure → obtain_failed + null members", async () => {
    ...
    try {
      ...
      await waitChildAgents(ctx.session, (c) => c.disposition === "ready", "ready before corrupt");
      ...
      assert.equal(ctx.session.getState().childAgents.disposition, "obtain_failed");
      assert.equal(ctx.session.getState().childAgents.members, null);
    } finally {
      await ctx.session.shutdown().catch(() => undefined);
      await fs.rm(ctx.home, { recursive: true, force: true }).catch(() => undefined);
    }
  ...
  it("reload/replay restores journaled identities + terminal Done/Failed", async () => {
    const ctx = await setup({ fixture: "child-agent-first-done", sessionId: "reload01" });
    try {
      await ctx.session.openWorkspace(ctx.workspace);
      const run = await ctx.session.prompt("go", "auto", { clientSessionId: "reload01" });
      await waitChildAgents(
        ctx.session,
        (c) => c.members?.some((m) => m.childId === "child-2" && m.status === "done") === true,
        "journaled done",
      );
      await ctx.session.shutdown().catch(() => undefined);
      ...
      try {
        ...
        const restored = await waitChildAgents(
          session2,
          (c) => c.disposition === "ready" && c.members?.some((m) => m.childId === "child-2" && m.status === "done") === true,
          "restored members",
        );
        assert.equal(restored.members?.length, 1);
        assert.equal(restored.members?.[0]?.identityLabel, "Fast");
        const replay = await session2.replayRun(run.runId, "reload01");
        assert.ok(replay.events.some((e) => e.type === "child_agent_update" && e.payload.kind === "child_agent_update"));
      } finally {
