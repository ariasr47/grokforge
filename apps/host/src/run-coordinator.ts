...
import type { ActivityRecord, ExecutionPhase, RunSnapshot, RunEventEnvelope, TerminalKind, FailureView, PolicySnapshot, ModelSnapshot } from "./run-types.js";
...
export class RunCoordinator {
  ...
  async admit(input:{sessionId:string;prompt:string;connectionGeneration:number;policy:PolicySnapshot;model:ModelSnapshot;executionPhase?:ExecutionPhase;dispatch?: (run:RunSnapshot)=>Promise<void>}):Promise<RunSnapshot>{
    const run=await this.withAdmissionLock(input.sessionId,async()=>{
      ...
      const next:RunSnapshot={sessionId:input.sessionId,runId:randomUUID(),connectionGeneration:input.connectionGeneration,state:"admitted",acceptedPrompt:input.prompt,admittedAt:now,updatedAt:now,lastEventSeq:0,policy:input.policy,model:input.model,terminalKind:null,finalAnswer:null,answerVouched:false,failure:null,executionPhase:input.executionPhase??"execute"};
  ...
  async appendAfterTerminalEvent(runId:string,payload:Extract<RunEventEnvelope["payload"],{kind:"plan_record"}|{kind:"decision_request"}>,type:Extract<RunEventEnvelope["type"],"plan_record"|"decision_request">):Promise<RunEventEnvelope>{
    return this.withRunMutationLock(runId,async()=>{
      let run=this.active.get(runId);
      if(!run){
        for(const snapshot of await this.journal.listSnapshots()){
