...
export class RunCoordinator {
  ...
  async hydrate(ownedSessionId?:string):Promise<void>{for(const run of await this.journal.listSnapshots()){if(ownedSessionId&&run.sessionId!==ownedSessionId)continue;const events=await this.journal.replay(run.sessionId,run.runId,0);let answer="", terminal:any=null;for(const e of events){if(e.sessionId!==run.sessionId||e.runId!==run.runId||e.connectionGeneration!==run.connectionGeneration)throw Object.assign(new Error("journal_unavailable"),{code:"journal_unavailable"});if(e.type!==e.payload.kind&&!(e.type==="run_terminal"&&e.payload.kind==="run_terminal"))throw Object.assign(new Error("journal_unavailable"),{code:"journal_unavailable"});if(e.payload.kind==="answer_delta")answer+=e.payload.delta;if(e.payload.kind==="run_state")run.state=e.payload.state;if(e.payload.kind==="run_terminal"){run.state="terminal";run.terminalKind=e.payload.terminalKind;run.finalAnswer=e.payload.finalAnswer;run.answerVouched=e.payload.answerVouched;run.failure=e.payload.failure;terminal=e.payload;}}run.lastEventSeq=events.at(-1)?.eventSeq??0;if(terminal)continue;this.active.set(run.runId,run);this.owners.set(run.sessionId,run.runId);this.answers.set(run.runId,answer);await this.finalize(run.runId,"failed",null,{code:"interrupted",message:"The host restarted before this run reached a terminal state.",retryable:true,recoveryAction:"retry_prompt"});}}
  async admit(input:{sessionId:string;prompt:string;connectionGeneration:number;policy:PolicySnapshot;model:ModelSnapshot;dispatch?: (run:RunSnapshot)=>Promise<void>}):Promise<RunSnapshot>{
    const run=await this.withAdmissionLock(input.sessionId,async()=>{
      ...
      const next:RunSnapshot={sessionId:input.sessionId,runId:randomUUID(),connectionGeneration:input.connectionGeneration,state:"admitted",acceptedPrompt:input.prompt,admittedAt:now,updatedAt:now,lastEventSeq:0,policy:input.policy,model:input.model,terminalKind:null,finalAnswer:null,answerVouched:false,failure:null};
  ...
  async appendOwnedEvent(runId:string,payload:RunEventEnvelope["payload"],type:RunEventEnvelope["type"],connectionGeneration?:number):Promise<RunEventEnvelope>{
    return this.withRunMutationLock(runId,async()=>{
      ...
      if(payload.kind==="answer_delta") this.answers.set(runId,(this.answers.get(runId)||"")+payload.delta);
  ...
  async finalize(runId:string,terminalKind:TerminalKind,finalAnswer:string|null=null,failure:FailureView=null):Promise<{won:boolean;run?:RunSnapshot}>{
    return this.withRunMutationLock(runId,async()=>{
      ...
      if(terminalKind==="answered"&&!candidate){terminalKind="failed";failure={code:"missing_final_answer",message:"No final answer was produced.",retryable:true,recoveryAction:"retry_prompt"};}
