...
test("terminal activity is published before run_terminal with no late activity", async()=>{
  ...
  await c.appendOwnedEvent(run.runId,{kind:"activity_update",activity:{activityId:"a",invocationId:"i",name:"tool",lifecycle:"terminal",execution:"executed",status:"succeeded",input:null,output:"ok",error:null,diff:null,policy,automaticEligibility:"not_eligible",autoApplied:false,editId:null,recovery:null}},"activity_update");
...
test("terminal runs reject every late payload without publication or mutation", async()=>{
  ...
  try {
    ...
    const activity:any={activityId:"a",invocationId:"i",name:"write",lifecycle:"terminal",execution:"executed",status:"succeeded",input:null,output:"late",error:null,diff:null,policy,automaticEligibility:"not_eligible",autoApplied:false,editId:null,recovery:null};
    const payloads:any[]=[
