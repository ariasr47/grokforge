...
export class RunJournal {
  ...
  async appendAfterTerminal(event:Omit<RunEventEnvelope,"eventSeq"|"occurredAt"> & Partial<Pick<RunEventEnvelope,"occurredAt">>):Promise<RunEventEnvelope>{
    if(!((event.type==="activity_update"&&event.payload.kind==="activity_update")||(event.type==="plan_record"&&event.payload.kind==="plan_record")||(event.type==="decision_request"&&event.payload.kind==="decision_request"))) throw Object.assign(new Error("only settlement activity may follow terminal"),{code:"run_terminal"});
