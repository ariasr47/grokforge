...
export type RunSnapshot = { sessionId:string; runId:string; connectionGeneration:number; state:RunState; acceptedPrompt:string; admittedAt:string; updatedAt:string; lastEventSeq:number; policy:PolicySnapshot; model:ModelSnapshot; terminalKind:TerminalKind|null; finalAnswer:string|null; answerVouched:boolean; failure:FailureView };
export type DecisionKind = "permission"|"diff"|"recovery_confirmation";
export type DecisionRequest = { requestId:string; invocationId:string; kind:DecisionKind; status:"pending"|"accepted"|"declined"|"expired"; title:string; detail:string; expiresAt:string|null; policy:PolicySnapshot };
export type ActivityRecord = { activityId:string; invocationId:string; name:string; lifecycle:"pending"|"terminal"; execution:null|"executed"|"not_executed"; status:"running"|"succeeded"|"failed"|"rejected"; input:unknown; output:unknown|null; error:string|null; diff:string|null; policy:PolicySnapshot; automaticEligibility:"read"|"fixed_inspection"|"text_edit"|"bypass"|"not_eligible"; autoApplied:boolean; editId:string|null; recovery:null|{kind:"guarded_revert";available:boolean;status:"available"|"pending"|"reverted"|"conflict"|"failed"} };
export type RunEventPayload = {kind:"run_started";run:RunSnapshot}|{kind:"run_state";state:Exclude<RunState,"terminal">;liveness:"provider"|"tool"|"decision"|"background"|"journal_recovery"|null}|{kind:"reasoning_delta";segmentId:string;delta:string}|{kind:"answer_delta";segmentId:string;delta:string}|{kind:"activity_update";activity:ActivityRecord}|{kind:"decision_request";request:DecisionRequest}|{kind:"run_terminal";terminalKind:TerminalKind;finalAnswer:string|null;answerVouched:boolean;failure:FailureView;terminalAt:string};
export type RunEventEnvelope = {schemaVersion:1;type:"run_started"|"run_state"|"reasoning_delta"|"answer_delta"|"activity_update"|"decision_request"|"run_terminal";sessionId:string;runId:string;eventSeq:number;connectionGeneration:number;occurredAt:string;payload:RunEventPayload};
